...
async function deleteSupabaseCourse(courseId) {
  ...
  /* The rest of what a course owns - its visual row, its visual job history and
     every rendered frame in the bucket. Shared with /api/course-orphans so the
     delete path and the "what got left behind" report cannot disagree about what a
     course consists of. Files go through the Storage API, never SQL: Supabase
     leaves a SQL-deleted object orphaned in the bucket, still stored and billed. */
  const purge = await purgeCourseData(supabaseBase(), supabaseKey(), courseId, { keepMapperJobs: true });
  console.log("course-maps: deleted", courseId, "-", JSON.stringify(purge));
