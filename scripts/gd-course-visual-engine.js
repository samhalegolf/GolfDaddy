(function(root,factory){
  ...
  function chooseExampleHoleCaptures(captures){
    var valid=(Array.isArray(captures)?captures:[]).filter(renderableCapture).filter(function(capture){
      var role=String(capture&&capture.role||"");
      return role!=="course-backdrop"&&!capture.terrainStageOnly;
    });
  ...
  function buildHoleFrameVisuals(captures,meta){
    ...
    (Array.isArray(captures)?captures:[]).filter(renderableCapture).forEach(function(capture){
      var role=String(capture&&capture.role||"");
      if(role==="course-backdrop"||capture.terrainStageOnly)return;
  ...
  /* Only imagery is left to split out: relief is derived server-side from the DEM in
     functions/lib/gd-relief-core.mjs, so the client never shades with a terrain capture. The
     server still PLANS terrain-reference captures for that DEM, so they must keep being held
     out of the stitch here. */
  function splitVisualCaptures(captures){
    var all=(Array.isArray(captures)?captures:[]).filter(renderableCapture);
    var imagery=all.filter(function(capture){return !(capture&&capture.terrainStageOnly);});
    return {all:all,imagery:imagery};
  }
  ...
  function buildCourseVisualMaster(courseId,opts){
    ...
    function finishBuild(record){
      ...
      var latestHasWorking=!!(latest&&(latest.rawMaster||latest.basicVisual||latest.previewVisual));
      var recordHasWorking=!!(record&&(record.rawMaster||record.basicVisual||record.previewVisual));
    ...
    var promise=Promise.resolve().then(function(){
      ...
      if(record.rawMaster&&record.rawMaster.captureSignature===signature&&existingRenderer===RENDERER_VERSION&&record.basicVisual&&existingAssetReady&&existingExampleReady&&existingHoleFramesReady){
      ...
      try{
        ...
        record.diagnostics=Object.assign({},record.diagnostics||{},{capturePlanSummary:capturePlanMeta,stitchOutputDimensions:{width:stitched.width,height:stitched.height},missingCoverage:stitched.missingAreas,sourceCaptureIds:stitched.sourceCaptureIds,holeFrameCount:record.holeFrameVisuals.length,holeFrames:record.holeFrameVisuals.map(function(asset){return {holeNumber:asset.holeNumber,path:asset.path,width:asset.width,height:asset.height,sourceCaptureIds:asset.sourceCaptureIds};}),exampleHole:record.exampleHoleVisual?{captureId:record.exampleHoleVisual.captureId,holeNumber:record.exampleHoleVisual.holeNumber,width:record.exampleHoleVisual.width,height:record.exampleHoleVisual.height}:null});
        record.versions=(record.versions||[]).concat([{version:version,type:"basic",rawMasterPath:record.rawMaster.path,basicImagePath:record.basicVisual.path,holeFramePaths:record.holeFrameVisuals.map(function(asset){return asset.path;}),sourceCaptureIds:stitched.sourceCaptureIds,createdAt:now(),metadata:Object.assign({},stitched.metadata||{},{holeFrameCount:record.holeFrameVisuals.length})}]);
        recordEvent(record,"course-visual-basic-ready",{version:version,width:stitched.width,height:stitched.height,missingAreas:stitched.missingAreas.length,holeFrameCount:record.holeFrameVisuals.length});
      }catch(error){
  ...
  function buildCourseVisualPreview(courseId,presetOrId,overrides,opts){
    ...
    return hydrateCourseVisualAssets(courseId).catch(function(){return null;}).then(function(){
      ...
      /* Bakes read the capture pixels - hydrate flattened captures here for the same reason
         the master build does. */
      return hydrateCaptureImages(allCaptures).then(function(){
      var split=splitVisualCaptures(allCaptures);
      if(scopedHole){
        record.presetId=preset.id;
  ...
  function publishCourseVisual(courseId,previewVersion){
    ...
    var overviewFinal=preview;
    var singleHoleFinal=record.singleHolePreviewVisual||record.exampleHoleVisual;
    ...
    var frameSources=(Array.isArray(record.holeFramePreviewVisuals)&&record.holeFramePreviewVisuals.length?record.holeFramePreviewVisuals:record.holeFrameVisuals||[]).filter(function(asset){return asset&&asset.dataUrl;});
  ...
  function visualCaptureRole(capture){
    ...
    return role==="course-backdrop"||role==="green-surround"||role==="play-corridor";
  }
  ...
  function buildFromCourseDatabase(courseId,opts){
    ...
    var plan=planCourseVisualCaptures(input);
