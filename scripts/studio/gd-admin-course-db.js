...
async function gdAdminCourseDbUpdate(courseId){
  ...
  try{
    ...
    if(engine&&typeof engine.buildFromCourseDatabase==="function")await engine.buildFromCourseDatabase(courseId,{enable3dBeta:beta3d,forceFresh:false});
...
function gdAdminCoursePreviewHoleCount(selected,record){
  const visualCount=Math.max(...[
    ...
    record&&record.holeFrameTerrainViews&&record.holeFrameTerrainViews.length||0,
...
function gdAdminCourseVisualNeedsAutoBuild(record,sourceStatus){
  ...
  if(beta3d&&!record.beta3dView)return true;
  return false;
}
...
function gdAdminCourseVisualSuppressPublishedFallback(record){
  ...
  const hasWorking=!!(record.rawMaster||record.basicVisual||record.exampleHoleVisual||record.previewVisual||record.terrainView||record.singleHolePreviewVisual||record.singleHoleTerrainView);
...
function gdAdminCourseVisualProducts(record){
  ...
  const overviewTerrain=gdAdminCourseVisualStageReady(record&&record.terrainView,"terrain-shading","native-visuals")?record.terrainView:null;
  ...
  const holeTerrain=gdAdminCourseVisualStageReady(record&&record.singleHoleTerrainView,"terrain-shading")?record.singleHoleTerrainView:null;
  const beta3dOn=!!(record&&record.courseOverrides&&record.courseOverrides.visualEngine&&record.courseOverrides.visualEngine.enable3dBeta);
  const suppressPublishedFallback=gdAdminCourseVisualSuppressPublishedFallback(record);
  const products=[
    {key:"course-overview",label:"Course overview",base:gdAdminCourseVisualProductBase(record),native:overviewNative,terrain:overviewTerrain,published:record&&record.publishedVisual||null,suppressPublishedFallback:suppressPublishedFallback},
    {key:"single-hole",label:"Single hole",base:record&&record.exampleHoleVisual||null,native:holeNative,terrain:holeTerrain,published:record&&record.singleHolePublishedVisual||null,suppressPublishedFallback:suppressPublishedFallback,holeNumber:record&&record.exampleHoleVisual&&record.exampleHoleVisual.holeNumber||record&&record.singleHoleTerrainView&&record.singleHoleTerrainView.holeNumber||null}
  ];
  if(beta3dOn||record&&record.beta3dView){
    products.push({key:"3d-beta",label:"3D view beta",beta:record&&record.beta3dView||null,stages:[{label:"Faux 3D",asset:record&&record.beta3dView||null}],holeNumber:record&&record.beta3dView&&record.beta3dView.metadata&&record.beta3dView.metadata.holeNumber||null});
  }
  return products;
}
...
function gdAdminCourseVisualProductCard(product){
  const visual=product.beta||product.terrain||product.native||(!product.suppressPublishedFallback?product.published:null)||product.base||null;
  ...
  const dims=width&&height?`${Math.round(width)}×${Math.round(height)}`:"";
  const activeStage=product.beta?"Faux 3D":product.terrain?"Terrain shading":product.native?"Native visuals":!product.suppressPublishedFallback&&product.published?"Published":product.base?"Base capture":product.suppressPublishedFallback?"Fresh run needed":"Waiting";
  const aspect=width&&height?width/height:0;
  const scrollClass=aspect&&(aspect<.35||aspect>3.25)?" scroll":"";
...
async function gdAdminCourseVisualBuildBasic(courseId){
  ...
  await engine.buildFromCourseDatabase(courseId,{enable3dBeta:overrides.visualEngine&&overrides.visualEngine.enable3dBeta===true});
  gdRenderAdminCourseDatabase();
  return false;
}
...
async function gdAdminCourseVisualRecapture(courseId){
  ...
  try{
    ...
    const buildOpts={enable3dBeta:overrides.visualEngine&&overrides.visualEngine.enable3dBeta===true,forceFresh:true,requireFreshCaptures:true};
    let record=null;
    let lastMessage="";
    ...
    await engine.buildCourseVisualPreview(courseId,presetId,overrides);
    record=gdAdminCourseVisualRecord(courseId)||engine.getRecord(courseId);
    if(!record.previewVisual||!record.singleHoleTerrainView){
