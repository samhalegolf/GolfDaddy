...
(async function run() {
  ...
  assert.equal(planRoles.includes("three-d-hole-beta"), false, "the 3D beta capture stage is gone (client-side 3D beta was removed)");
  ...
  assert.equal(preview.terrainView, undefined, "no client-side terrain product is baked (relief is derived server-side now)");
  ...
  assert.equal(preview.holeFrameTerrainViews, undefined, "hole frames have no separate terrain product");
  assert.equal(preview.holeFramePreviewVisuals[0].metadata.playSurface.useGpsPlayFraming, true, "hole frame preview keeps play-surface metadata");
  ...
  assert.equal(published.publishedVisual.dataUrl, published.previewVisual.dataUrl, "published overview uses the native-visual product");
  assert.ok(published.singleHolePublishedVisual && published.singleHolePublishedVisual.dataUrl === published.singleHolePreviewVisual.dataUrl, "publish includes the native single-hole product");
  ...
  const plannedBuilt = await engine.buildFromCourseDatabase("planned-course");
  ...
  assert.ok(visualRequests.filter((request) => request.role === "play-corridor").every((request) => request.captureLens === "mobile-hole" && request.lensAspectRatio === 9 / 16), "corridor capture requests carry the mobile-hole lens into the browser capture step");
  assert.ok(visualRequests.filter((request) => request.role === "play-corridor").every((request) => request.lensOrientation === "play-axis" && request.includeAnchorPins === true), "mobile-hole capture requests are aimed by the hole axis and include the donor route");
  ...
  assert.equal(plannedBuilt.terrainView, undefined, "no terrain product is derived client-side");
  assert.equal(plannedBuilt.beta3dView, undefined, "no 3D beta product is derived client-side");
