...
(function (root) {
  ...
  function draw(canvas, surface, project, options) {
    ...
    /* Bands first, for the same reason the SVG does it first: they are ground under the lines. */
    var bands = drawing.bands || [];
    for (var bi = 0; bi < bands.length; bi++) {
      var band = bands[bi], q = [], ok = true;
      for (var qi = 0; qi < band.quad.length; qi++) {
        var qp = toPx(band.quad[qi]);
        if (!qp) { ok = false; break; }
        q.push(qp);
      }
      if (!ok || band.alpha <= 0.01) continue;
      ctx.beginPath();
      ctx.moveTo(q[0].x, q[0].y);
      for (var qj = 1; qj < q.length; qj++) ctx.lineTo(q[qj].x, q[qj].y);
      ctx.closePath();
