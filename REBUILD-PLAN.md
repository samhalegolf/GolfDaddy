...
Wand map checkpoint: see `docs/reports/GREEN_WAND_ENGINE_EXTRACTION_MAP.md`. Current classification: detection engine lives in `scripts/gd-app-core.js` as `window.GolfDaddyGreenWandEngine.analyzeGreenWand`; adapters live around green-centred Leaflet tile crop/projection and scale-lock helpers in core; standalone UI lives in core plus `gd-wand-flow-layers-v1.js`; diagnostics live mostly inert in `gd-wand-belt-layers-v1.js`; obsolete guards live in `gd-wand-belt-layers-v1.js`, `gd-wand-flow-layers-v1.js`, `gd-route-audit.js`, and mapper wrappers in `gd-course-library-pin-lock.js`. Source script count before this mapping: 59; after this mapping: 59. Core line count before this mapping: 26,977; after this mapping: 26,977. Extraction not yet completed in the mapping commit. Exact next step: run/add Green Shape Engine behavior fixtures against the current engine, then extract the detector block into one engine owner file only if fixture output stays stable.
