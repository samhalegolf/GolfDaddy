...
Next Phase B target: wand-in-core should be mapped before editing, with the locked Green Wand baseline in `package.json` treated as move-only unless Sam explicitly reopens behavior. After that, course picker responsibilities remain a separate map/carve; do not mix either with shell consolidation.
...
Wand map checkpoint: see `docs/reports/GREEN_WAND_ENGINE_EXTRACTION_MAP.md`. Current classification: detection engine now lives in `scripts/gd-green-shape-engine.js` as `window.GolfDaddyGreenWandEngine.analyzeGreenWand` with `window.GDGreenShapeEngine` as the future-facing alias; adapters live around green-centred Leaflet tile crop/projection and scale-lock helpers in core; standalone UI lives in core plus `gd-wand-flow-layers-v1.js`; diagnostics live mostly inert in `gd-wand-belt-layers-v1.js`; obsolete guards live in `gd-wand-belt-layers-v1.js`, `gd-wand-flow-layers-v1.js`, `gd-route-audit.js`, and mapper wrappers in `gd-course-library-pin-lock.js`. Source script count before extraction: 59; after extraction: 60. Core line count before extraction: 26,977; after extraction: 26,322. Extraction completed without wiring AutoMapper automatic acceptance. Exact next step: decide whether `gdValidateWandOutline` moves into the engine contract or remains adapter validation, then replace the mapper's standalone Wand fallback with an explicit AutoMapper candidate-crop/refinement adapter.
